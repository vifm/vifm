...
int fops_delete(view_t *view, int reg, int use_trash);
 ...
 * flag. */
int fops_mkdirs(view_t *view, int at, char *names[], int count,
		int create_parent);
...
int fops_mkfiles(view_t *view, int at, char *names[], int count);

/* Returns new value for save_msg flag. */
int fops_restore(view_t *view);

/* Initiates background calculation of directory sizes.  Forcing disables using
 * previously cached values. */
void fops_size_bg(const view_t *view, int force);
