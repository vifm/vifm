 ...
 * save_msg flag. */
int fops_cpmv(view_t *view, char *list[], int nlines, CopyMoveLikeOp op,
		int force);

/* Replaces file specified by dst with a copy of the current file of the
 * view. */
void fops_replace(view_t *view, const char dst[], int force);

/* Copies or moves marked files to the other view in background.  Returns new
 * value for save_msg flag. */
int fops_cpmv_bg(view_t *view, char *list[], int nlines, int move, int force);
