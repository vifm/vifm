...
typedef struct menu_data_t
{
	...
	KHandlerResponse (*key_handler)(view_t *view, struct menu_data_t *m,
			const wchar_t keys[]);

	/* Callback that is called when menu item is selected.  Should return non-zero
	 * to stay in menu mode. */
	int (*execute_handler)(view_t *view, struct menu_data_t *m);
...
void menus_init_data(menu_data_t *m, view_t *view, char title[],
		char empty_msg[]);
 ...
 * returned. */
int menus_goto_file(menu_data_t *m, view_t *view, const char spec[],
		int try_open);

/* Navigates to directory from a menu. */
void menus_goto_dir(view_t *view, const char path[]);
 ...
 * returned. */
char * menus_get_targets(view_t *view);
 ...
 * next. */
KHandlerResponse menus_def_khandler(view_t *view, menu_data_t *m,
		const wchar_t keys[]);
