...
delmarks_cmd(const cmd_info_t *cmd_info)
{
	...
	for(i = 0; i < cmd_info->argc; i++)
	{
		...
		for(j = 0; cmd_info->argv[i][j] != '\0'; j++)
		{
			clear_mark(curr_view, cmd_info->argv[i][j]);
		}
	}
	return 0;
}
...
mark_cmd(const cmd_info_t *cmd_info)
{
	...
	if(cmd_info->qmark)
	{
		if(!is_mark_empty(curr_view, mark))
		{
	...
	if(cmd_info->argc == 1)
	{
		...
		return set_user_mark(curr_view, mark, entry->origin, entry->name);
	}
	...
	result = set_user_mark(curr_view, mark, expanded_path, file);
	free(expanded_path);
