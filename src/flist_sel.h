...
void flist_sel_restore(view_t *view, reg_t *reg);

/* Counts number of selected files and writes saves the number in
 * view->selected_files. */
void flist_sel_recount(view_t *view);

/* Selects or unselects entries in the given range. */
void flist_sel_by_range(view_t *view, int begin, int end, int select);
...
int flist_sel_by_filter(view_t *view, const char cmd[], int erase_old,
		int select);
 ...
 * statusbar. */
int flist_sel_by_pattern(view_t *view, const char pattern[], int erase_old,
		int select);

/* Selects up to count elements starting at position at (or current cursor
 * position, if at is negative). */
void flist_sel_count(view_t *view, int at, int count);
...
int flist_sel_range(view_t *view, int begin, int end, int select_current);
