...
int fops_view_can_be_changed(const view_t *view);
...
char ** fops_grab_marked_files(view_t *view, size_t *nmarked);
...
void fops_fixup_entry_after_rename(view_t *view, dir_entry_t *entry,
		const char new_fname[]);

/* Adds marked files to the ops.  Considers UI cancellation.  dst_hint can be
 * NULL.  Returns number of files enqueued. */
int fops_enqueue_marked_files(ops_t *ops, view_t *view, const char dst_hint[],
		int to_trash);
...
int fops_can_read_selected_files(view_t *view);
...
int fops_check_dir_path(const view_t *view, const char path[], char buf[],
		size_t buf_len);
...
void fops_prepare_for_bg_task(view_t *view, bg_args_t *args);
...
void fops_append_marked_files(view_t *view, char buf[], char **fnames);
 ...
 * otherwise zero is returned. */
int fops_view_can_be_extended(const view_t *view, int at);
...
const char * fops_get_dst_dir(const view_t *view, int at);
