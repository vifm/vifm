...
/* Fills the buffer with string representation of owner user for the entry. */
void get_uid_string(const dir_entry_t *entry, size_t buf_len, char buf[]);

/* Fills the buffer with string representation of owner group for the entry. */
void get_gid_string(const dir_entry_t *entry, size_t buf_len, char buf[]);
