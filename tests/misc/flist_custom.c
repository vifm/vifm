...
TEST(empty_list_is_not_accepted)
{
	...
	assert_false(flist_custom_finish(&lwin, CV_REGULAR) == 0);
}
...
TEST(files_are_sorted_undecorated)
{
	...
	assert_success(flist_custom_finish(&lwin, CV_REGULAR));
...
setup_custom_view(FileView *view, int very)
{
	...
	snprintf(view->curr_dir, sizeof(view->curr_dir), "%s/existing-files",
			test_data);
	assert_true(flist_custom_finish(view, very ? CV_VERY : CV_REGULAR) == 0);
}
