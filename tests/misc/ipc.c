...
static void test_ipc_callback(char *args[]);
static void test_ipc_callback2(char *args[]);
...
{
	ipc_t *const ipc = ipc_init(NAME, &test_ipc_callback);
	assert_non_null(ipc);
	ipc_free(ipc);
}
...
{
	ipc_t *const ipc = ipc_init(NULL, &test_ipc_callback);
	assert_non_null(ipc);
	ipc_free(ipc);
}
...
TEST(message_is_delivered, IF(enabled_and_not_in_wine))
{
	...
	ipc_t *const ipc1 = ipc_init(NAME, &test_ipc_callback);
	ipc_t *const ipc2 = ipc_init(NAME, &test_ipc_callback2);
