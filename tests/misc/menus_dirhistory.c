...
TEST(history_of_a_view_is_listed_most_recent_first)
{
	...
	flist_hist_setup(&lwin, SANDBOX_PATH, "file1", 0);
	flist_hist_setup(&lwin, TEST_DATA_PATH, "file2", 0);
...
TEST(history_does_not_contain_duplicated_entries)
{
	...
	flist_hist_setup(&lwin, SANDBOX_PATH, "file1", 0);
	flist_hist_setup(&lwin, TEST_DATA_PATH, "file2", 0);
	flist_hist_setup(&lwin, SANDBOX_PATH, "file3", 0);
	flist_hist_setup(&lwin, TEST_DATA_PATH, "file4", 0);
...
TEST(position_of_current_directory_is_determined_in_custom_view)
{
	...
	flist_hist_setup(&lwin, TEST_DATA_PATH, "file2", 0);
	flist_hist_setup(&lwin, lwin.curr_dir, "file1", 0);
	flist_hist_setup(&lwin, SANDBOX_PATH, "file3", 0);
