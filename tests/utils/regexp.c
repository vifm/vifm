...
{
	assert_string_equal("barfoobar", regexp_replace("barfoobar", "*foo", "z", 1));
}

TEST(no_infinite_loop_on_empty_global_match)
{
	assert_string_equal("zbarfoobar", regexp_replace("barfoobar", "", "z", 1));
}
...
{
	assert_string_equal("10", regexp_replace("100", "0$", "", 1));
}

TEST(back_reference_substitution)
{
	assert_string_equal("barfoobaz",
			regexp_replace("bazfoobar", "(.*)foo(.*)", "\\2foo\\1", 1));

	assert_string_equal("barbaz", regexp_replace("foobaz", "foo", "bar\\", 1));

	assert_string_equal("f0t0tbaz", regexp_replace("foobaz", "o", "0\\t", 1));
}
